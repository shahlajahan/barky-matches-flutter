...
| **4.8** | Flutter migration — **two parts, scope clarified in Revision 5 (§9, §17):** (a) read side — Marketplace browse/detail data-source classes switch to `marketplaceListing.js`; (b) **write side, new** — Add Product create/edit path (`add_product_page.dart`, `Product.toJson()`) gains `productInputRevision` (absent parsed as `0`, computed +0/+1 on every write) and preserves any existing server-owned compliance field across an ordinary edit, closing the exact full-document-overwrite gap Slice 4.2's Phase A contract documents | Slice 4.5 (read side); Slice 4.2 (write side) | Slice-7-adjacent, **not started**; files not yet located/verified beyond the write-side files already identified in §9 (`lib/models/product.dart`, `lib/ui/business/petshop/add_product_page.dart`) | Read side: functional parity vs. direct-query baseline in staging. Write side: create sends `productInputRevision: 0`; edit preserves/increments it correctly; edit to a product carrying server-owned fields leaves them byte-for-byte unchanged (§17 steps 13-14) |
| **4.9** | Final Rules lock-down + production activation: read-gate + approval-transition-lock **and `productInputRevision` Phase B strict-presence upgrade** (§9 Revision 5) deployed to `firestore.rules`; real policy version activated | Slice 4.2, 4.4, 4.8 (**both** parts), and the Turkish legal content decision (§19) | Yes (mechanism); **activation itself blocked** on legal content | Full Rules-emulator suite: direct public reads denied, owner/admin reads unaffected, approval matrix confirmed, **and** strict `productInputRevision` presence enforced with zero regression (every existing product already backfilled per §17 steps 15-16) |
...
10. Trigger 4.7's post-activation bulk recompute pass — see step 22 below, which applies here too: bootstrap, not just ordinary activation, needs it.
...
13. **Implement and ship Slice 4.8's write-side Flutter change to staging clients** (Revision 5 correction 33 — this is a scope clarification of Slice 4.8, which previously named only the read-side browse/detail endpoint migration, §16): add `productInputRevision` to the `Product` model; parse an absent stored value as `0`; send/preserve it on every full-document write; compute the transaction-safe +0/+1 value against the previously-read document, exactly per §9's Phase A→Phase B transition rules; and preserve any existing server-owned compliance field across an ordinary edit — either by having the write path read-and-reinclude those fields, or by changing the write strategy so a full-document overwrite can no longer silently drop fields it doesn't itself know about. **This step, and its staging verification (step 14), must complete before step 15 below — not after.**
14. Verify the upgraded write-side client in staging: an ordinary create sends `productInputRevision: 0`; an ordinary edit preserves and correctly increments it; an edit to a product already carrying server-owned compliance fields (seeded via Admin SDK for this test) leaves them byte-for-byte unchanged.
15. **Only now** run the `productInputRevision` backfill for real in staging (Admin SDK, bypasses Rules): set `productInputRevision: 0` on every existing staging product still missing it. Running this before step 13/14 would backfill a value the very next un-upgraded client's ordinary edit could either strip (if Phase A's row D didn't exist) or, correctly, be rejected by (§9's row D) — rejecting a seller's ordinary edit is exactly the outcome this ordering avoids by not backfilling until the writer that would otherwise collide with it has already shipped.
16. Verify: no product in staging still has an absent `productInputRevision`.
17. Run 4.7's migration sweep for real in staging (writes enabled: initial `productComplianceDecisions` for every existing staging product) — still zero customer-visible impact, since no Rules changed yet beyond Phase A.
18. Verify: re-measure coverage, spot-check a sample.
19. Deploy 4.5 (`getMarketplaceProductList`/`Detail`) to staging — exported, **behind its own disabled flag**.
20. Switch Flutter clients (staging build, 4.8's read-side change) to the new endpoints; verify functional parity.
21. Deploy 4.9's Rules lock-down — **both** the read-gate/approval-transition-lock **and** `productInputRevision`'s Phase B strict-presence upgrade (§9) — to staging, only now: every existing product already carries `productInputRevision` (step 15/16) and an accurate, migrated decision (step 17/18), so strict presence enforcement introduces no regression.
22. Repeat steps 5–21 against production, same order, only after staging fully verifies. Steps 1–4 may deploy to *production* safely even before real legal content is ready — nothing in them is reachable or active yet. Step 5 onward (real bootstrap/activation) is blocked on the Turkish legal content decision (§19/§20).
23. Flip 4.4's flag on in production only *after* production's Rules lock-down (step 21-equivalent) has completed — approval enforcement must never activate before the read-gate itself is live.
24. **Immediately following any policy activation — bootstrap or ordinary (steps 8/17/22, and any future re-activation) — trigger 4.7's bulk recompute pass proactively (Revision 3 correction 20)** — rather than relying solely on the ordinary bounded scheduled sweep — to minimize the listings-thin-out window the corrected fail-closed design otherwise produces (every decision computed under a retired policy version correctly, and immediately, fails the live evaluator's freshness check once the pointer is read fresh, §10.1).

**Explicitly prohibited: any `productInputRevision` backfill or `productComplianceDecisions` migration write before step 13/14's Flutter write-side preservation change has shipped and been verified.** Migrating data into a write path that will immediately strip it back out on the next ordinary seller edit is worse than not migrating at all — it converts a known, documented dormant gap (§9, row A) into an intermittent, hard-to-reproduce one.

**No compliance output becomes authoritative anywhere in this sequence before step 21/22's Rules lock-down, and even then only for the specific enforcement each of Slices 4.3–4.9 individually governs** — Slice 4.2's own Phase A contract never makes any compliance field authoritative on its own; it only prevents regression and enables monotonic adoption (§9).
