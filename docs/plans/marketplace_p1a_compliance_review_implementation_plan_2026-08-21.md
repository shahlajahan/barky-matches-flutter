...
### `complianceDocumentScopes/{scopeId}` — corrected (correction 9); `sellerRelationship` denormalized (Revision 7 correction 40)

Gains `verifiedBrandId` for `scopeType: 'brand'` scopes. Gains `sellerRelationship`, denormalized from the source document (Revision 7).
...
### Source-document verification and the reconciled ≤8-operation / ≤42-read calculation (Revision 6 correction 35; extended Revision 7 correction 42)
...
**The already-budgeted "matched-scope document resolution" step is source-document verification, not a bare read (Revision 7 correction 42).** For each of up to `MATCHED_SCOPE_CAP=10` scopes selected across all seven lookups, the recompute reads that scope's source `complianceDocuments/{documentId}` — the only possible origin of `expiresAt`/`validUntil`, since `complianceDocumentScopes` itself carries no such field (§4). At this same, already-budgeted read, the recompute additionally verifies, at zero extra read cost:
...
### 13.1 Exact Slice 4 file plan — sub-slices 4.1-4.9 (Revision 3; Slice 4.1 row completed in Revision 4; Slice 4.3 rows completed in Revision 6; two prerequisite corrective sub-passes added, Revision 7)

**Revision 7 correction 45 — two new, separately-reviewed corrective sub-passes must land, in order, before Slice 4.3 resumes.** Neither is folded into Slice 4.2 or Slice 4.3's own rows below — each is its own commit, its own review, its own verification, exactly like every other slice in this table:
...
| 4.3 | New | `functions/src/marketplace/compliance/complianceMatching.js` | The 8-operation/42-read bounded lookup, exact seven-query mapping — **relationship-filtered, Revision 7 correction 41** (§10), `productEvidenceLinks` writer using the frozen `deriveEvidenceLinkId` formula (§4) | **Slice 4.1, 4.2, the Slice 3 correction (Revision 7), and the Slice 4.2 correction (Revision 7)** — corrected again, Revision 7: the two corrective sub-passes above must land first, since the seven queries and the matching-field set both depend on their output |
...
| **4.3** | Matching/evaluator engine: `complianceMatching.js`, `complianceProductRecompute.js`, `complianceEligibilityEvaluator.js`, `complianceBrandNormalizer.js`, plus additive constants (`complianceConstants.js`) and the two required composite indexes (`firestore.indexes.json`, §14) — **contract completed in Revision 6, relationship-filtered in Revision 7**: exact seven-query mapping (now including `sellerRelationship`), exact `productEvidenceLinks` schema (`matchedVia`, §4), exact deterministic link-ID/cleanup design, exact source-document verification (§10), and the exact index pair, all frozen | Slice 4.1, 4.2, **the Slice 3 correction (Revision 7), and the Slice 4.2 correction (Revision 7)** — corrected again, Revision 7 correction 45: the two corrective sub-passes above are new hard prerequisites, since the seven queries and the matching-field set both depend on their output | Yes — unexported internal modules | Full Revision 6 test list (§15), extended by Revision 7's product-level `sellerRelationship` test list (§15): exact seven-query-shape tests (relationship-filtered); cross-tenant **and cross-relationship** isolation per lookup type; deterministic limit/tie-break tests, including the false-negative regression (three wrong-relationship candidates cannot hide a valid fourth); ≤8-operation/≤42-read bound test; both Revision 7 composite-index declarations verified; fresh-pointer evaluation on consecutive calls; epoch/policyVersion/productInputRevision/expiry/activeEvidenceRefs freshness validation; `productEvidenceLinks` never read by the evaluator; deterministic link-ID tests; prior-link delete/recreate without a discovery query; cap-10 behavior; transaction reads-before-writes; retry determinism; product/scope/source-document relationship triple-equality; no default/backfill relationship; no `functions/index.js` export; no activation/migration/backfill/Rules-gate/Slice-4.4+ behavior |
...
4. **Implement, review, and commit the Slice 4.2 correction (Revision 7 correction 45): `sellerRelationship` added to the dormant product schema, with its own absence/presence contract and its inclusion in the `productInputRevision` matching-field set (§9).** A separate commit from Slice 4.2's own original, already-committed work — never amended into it. Steps 3 and 4 are hard prerequisites for step 5 — Slice 4.3's seven queries and matching-field set both depend on their output (§13.1 correction 45).
5. Deploy 4.3 (matching/evaluator engine, relationship-filtered per Revision 7 correction 41) — **unexported internal modules**, genuinely uncallable. Kept unexported/non-authoritative exactly like every prior Slice 4.3 posture — nothing in this rollout changes that.
