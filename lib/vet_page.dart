...
final List<VetTip> _localTips = [
  VetTip(text: "🐾 Regular check-ups keep your dog healthy", category: "health"),
  VetTip(text: "💉 Vaccinations are essential for prevention", category: "health"),
...
final String? instagram = contact['instagram']?.toString();
final String? website = contact['website']?.toString();
...
final veterinary =
    Map<String, dynamic>.from(sectorData['veterinary'] ?? {});

// ✅ THEN use it
final profileContent =
    Map<String, dynamic>.from(veterinary['profileContent'] ?? {});
...
debugPrint("🔥 LOGO URL: $logoUrl");
        ...
        final workingHours =
            Map<String, dynamic>.from(veterinary['workingHours'] ?? {});
...
} else if (rawWorkingHours is String && rawWorkingHours.isNotEmpty) {
  workingHoursMap = {
    "hours": rawWorkingHours,
  };
}
        final services =
            Map<String, dynamic>.from(veterinary['services'] ?? {});
        ...
        final specialtyText =
            (profileContent['specialties'] ?? '').toString().trim();
    ...
    (data['isPartner'] ?? veterinary['featuredVet'] ?? false) == true;
    debugPrint("🔥 BIO = ${profileContent['bio']}");
        return VetCardData(
          id: doc.id,
          name: (profile['displayName'] ?? 'Vet').toString(),
...
logoUrl: logoUrl,
          ...
          description: (profileContent['bio'] ?? '').toString(),
...
coverImageUrl: displayImage,
  ...
  double? _extractLng(
    ...
    final candidates = [
      ...
      veterinary['location'] is Map ? veterinary['location']['longitude'] : null,
      contact['lng'],
      contact['longitude'],
    ];
...
Widget build(BuildContext context) {
  ...
  /// ✅ بعد details
  if (appState.selectedVet != null) {
    return VetDetailsPage(vet: appState.selectedVet!);
  }
