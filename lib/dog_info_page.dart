...
class DogInfoPage extends StatelessWidget {
  ...
  Future<ImageProvider?> _loadDogImage() async {
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint(
          'DogInfoPage - Image file does not exist: ${dog.imagePaths[0]}',
        );
      }
      return null;
    } catch (e) {
      if (kDebugMode) {
        debugPrint(
          'DogInfoPage - Error loading image from path ${dog.imagePaths[0]}: $e',
        );
      }
      return null;
    }
  ...
  String _translateBreed(BuildContext context, String breedKey) {
    ...
    switch (breedKey) {
      ...
      case 'breedCavalierKingCharlesSpaniel':
        return l10n?.breedCavalierKingCharlesSpaniel ??
      ...
      case 'breedGermanShorthairedPointer':
        return l10n?.breedGermanShorthairedPointer ??
      ...
      case 'breedStaffordshireBullTerrier':
        return l10n?.breedStaffordshireBullTerrier ??
  ...
  String _translateTrait(BuildContext context, String traitKey) {
    ...
    return rawToLocalized[lowerTrait] ??
        (traitKey.startsWith('trait') ? traitKey.substring(5) : traitKey);
  }
  ...
  Future<void> _schedulePlaydate(BuildContext context) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          '${l10n?.dogInfoPlaydateScheduled ?? 'Playdate scheduled with'} ${dog.name}',
        ),
      ),
    );
    if (kDebugMode) {
  ...
  Widget build(BuildContext context) {
    ...
    if (kDebugMode) {
      debugPrint(
        'DogInfoPage - Building UI for dog: ${dog.name}, ID: ${dog.id}',
      );
    }

    return Scaffold(
      appBar: AppBar(title: Text(dog.name), backgroundColor: Colors.pink[300]),
      body: Container(
                      ...
                      return ClipRRect(
                        ...
                        child: Image(
                          ...
                          errorBuilder: (context, error, stackTrace) {
                            if (kDebugMode) {
                              debugPrint(
                                'DogInfoPage - Error loading image for ${dog.name}: $error',
                              );
                            }
                            return const Icon(
                              Icons.pets,
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              '${l10n?.dogInfoDisliked ?? 'You disliked'} ${dog.name}',
                            ),
                          ),
                        );
                        if (kDebugMode) {
                      ...
                      tooltip:
                          l10n?.dogInfoDislikeTooltip ?? 'Dislike this dog',
                    ),
                    IconButton(
                      ...
                      onPressed: () {
                        ...
                        if (kDebugMode) {
                          debugPrint(
                            'DogInfoPage - Initiated chat for ${dog.name}',
                          );
                        }
                      },
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        ...
                        if (kDebugMode) {
                          debugPrint(
                            'DogInfoPage - Toggled favorite for ${dog.name}, isFavorite: $isFavorite',
                          );
                        }
                      },
                      tooltip:
                          l10n?.dogInfoAddFavoriteTooltip ?? 'Add to favorites',
                    ),
                    IconButton(
                      icon: const Icon(
                      ...
                      tooltip:
                          l10n?.dogInfoSchedulePlaydateTooltip ??
                          'Schedule a playdate',
                    ),
                  ],
                ),
