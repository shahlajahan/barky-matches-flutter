...
import 'package:barky_matches_fixed/app_state.dart'; // 👈 مهم
...
final VoidCallback? onOpenFullProfile; // 🔥 NEW
  ...
  // ─────────────────────────────
  Widget _buildInfo() {
  final about = (data.description != null && data.description!.trim().isNotEmpty)
  ...
  return SingleChildScrollView(
    child: Column(
      ...
      children: [
        ...
        Text(
          about,
          style: AppTheme.caption(color: Colors.white70).copyWith(
            height: 1.5,
          ),
        ),
      ],
  ...
  if (data.isPartner) {
    ...
    context.read<AppState>().openVetDetails(data); // 🔥 این
  } else {
    onClose?.call();
