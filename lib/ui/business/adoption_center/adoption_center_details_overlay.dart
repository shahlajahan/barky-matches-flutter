...
class AdoptionCenterDetailsOverlay extends StatefulWidget {
  ...
  final ValueChanged<AdoptionPetModel>? onOpenPet;
...
class _AdoptionCenterDetailsOverlayState
  ...
  String? get _heroImageUrl {
    ...
    final profileContent = Map<String, dynamic>.from(
      _adoptionData['profileContent'] ?? _adoptionData['profile'] ?? {},
    );

    final candidates = <String>[
      ..._stringList(_adoptionData['coverImageUrl']),
      ..._stringList(_adoptionData['coverImage']),
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> _getReviewsStream() {
    final collection = FirebaseFirestore.instance
        ...
        .where('businessType', isEqualTo: 'adoptionCenter');

    if (_sortType == ReviewSortType.newest) {
      debugPrint(
        'ADOPTION REVIEWS QUERY businessId=${widget.data.id} '
        'sort=NEWEST orderBy=createdAt',
      );
    ...
    debugPrint(
      'ADOPTION REVIEWS QUERY businessId=${widget.data.id} '
      'sort=MOST_RELEVANT orderBy=rankScore',
    );
  ...
  Future<void> _openWriteReviewSheet({
    ...
    debugPrint(
      'REVIEW SHEET OPEN source=adoption_center businessId=${widget.data.id} '
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (_) => _WriteReviewSheet(adoptionCenterId: widget.data.id),
    ).whenComplete(() {
      debugPrint(
        'REVIEW SHEET CLOSED source=adoption_center businessId=${widget.data.id}',
      );
      FocusManager.instance.primaryFocus?.unfocus();
    });
  }

  void _openAvailablePetsTab() {
  ...
  Widget _buildHeader() {
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final profileContent = Map<String, dynamic>.from(
          adoptionData['profileContent'] ?? adoptionData['profile'] ?? {},
        );
...
*/
  ...
  Widget _buildOverviewTab() {
    ...
    final aboutCandidates = <MapEntry<String, String>>[
      MapEntry('profile.bio', (profile['bio'] ?? '').toString().trim()),
      MapEntry(
        'profile.description',
        (profile['description'] ?? '').toString().trim(),
      ),
      MapEntry(
        'sectorData.adoptionCenter.description',
        (adoptionData['description'] ?? '').toString().trim(),
      ),
      MapEntry(
        'sectorData.adoptionCenter.profile.bio',
        (profileContent['bio'] ?? '').toString().trim(),
      ),
      MapEntry(
        'adoption_center.description',
        (sectorData['adoption_center'] is Map
                ? (sectorData['adoption_center'] as Map)['description']
                : '')
            .toString()
            .trim(),
      ),
    ...
    final selectedAbout = aboutCandidates.firstWhere(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          debugPrint(
            ...
            'status=${AdoptionPetStatus.available} isVisible=true '
            'error=${snapshot.error}',
          );
          return Center(
            child: Text(
        ...
        debugPrint(
          ...
          'collection=adoption_pets businessId=${widget.data.id} '
          'status=${AdoptionPetStatus.available} isVisible=true '
          'count=${docs.length} ids=${docs.map((doc) => doc.id).join(',')}',
        );
    ...
    return ListView(
      ...
      children: [
        _sectionCard(
          ...
          child: Row(
            children: [
              _contactButton(LucideIcons.phone, 'Call', widget.onCall),
              const SizedBox(width: 10),
              _contactButton(
        ...
        _sectionCard(
          title: 'Location',
          child: Text(
            _locationText.isNotEmpty ? _locationText : widget.data.address,
            style: AppTheme.bodyMedium(color: Colors.black87),
          ),
        ),
      ],
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final images = <String>{
          ...
          ..._stringList(adoptionData['coverImageUrl']),
                          ...
                          child: Padding(
                            ...
                            child: SizedBox(
                              ...
                              child: ElevatedButton(
                                onPressed: _openAvailablePetsTab,
...
class _WriteReviewSheet extends StatefulWidget {
  ...
  const _WriteReviewSheet({required this.adoptionCenterId});
...
class _WriteReviewSheetState extends State<_WriteReviewSheet> {
  ...
  final FocusNode _reviewFocusNode = FocusNode(
    debugLabel: 'adoption center review field',
  );
  ...
  void _unfocusReviewSheet() {
    ...
    debugPrint(
      'REVIEW SHEET UNFOCUS source=adoption_center '
  ...
  void dispose() {
    debugPrint(
      'REVIEW SHEET DISPOSE source=adoption_center '
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('reviews').add({
        'businessId': widget.adoptionCenterId,
        'businessType': 'adoptionCenter',
        'userId': user.uid,
        'userName':
