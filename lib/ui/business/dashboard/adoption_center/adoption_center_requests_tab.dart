...
class AdoptionCenterRequestsTab extends StatefulWidget {
  ...
  const AdoptionCenterRequestsTab({super.key, required this.businessId});
...
class _AdoptionCenterRequestsTabState extends State<AdoptionCenterRequestsTab> {
  int _tab = 0;
    ...
    return SafeArea(
      ...
      child: Column(
        children: [
          ...
          Flexible(child: _buildList(status: _statuses[_tab])),
        ],
      ),
    );
  }
  ...
  Widget _buildTabs() {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            _tabButton(title: "Pending", index: 0),

            _tabButton(title: "Approved", index: 1),

            _tabButton(title: "Rejected", index: 2),
          ],
        ),
      ),
    );
  ...
  Widget _buildList({required String status}) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        debugPrint("🐾 REQUEST ERROR = ${snapshot.error}");

        debugPrint("🐾 REQUEST DOC COUNT = ${snapshot.data?.docs.length}");
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final form = Map<String, dynamic>.from(data["form"] ?? {});
            ...
            final dogPhoto = data["dogPhoto"];

            final requester = personal["fullName"] ?? "Unknown";

            final busy = _busyRequestId == doc.id;
