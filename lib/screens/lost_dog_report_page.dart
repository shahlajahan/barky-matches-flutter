...
class LostDogReportPage extends StatefulWidget {
  ...
  @override
  _LostDogReportPageState createState() => _LostDogReportPageState();
}
  ...
  Future<void> _getCurrentLocation() async {
    try {
      ...
      if (!serviceEnabled) {
        ...
        if (kDebugMode) print('LostDogReportPage - Using default position due to disabled location services');
        return;
      }
      ...
      if (permission == LocationPermission.denied || permission == LocationPermission.deniedForever) {
        ...
        if (permission == LocationPermission.denied || permission == LocationPermission.deniedForever) {
          ...
          if (kDebugMode) print('LostDogReportPage - Using default position due to permission denied');
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Location permission denied. Please enable it manually.')),
          );
          return;
        }
      }
      ...
      final position = await Geolocator.getCurrentPosition(
        ...
        timeLimit: const Duration(seconds: 30), // افزایش زمان به 30 ثانیه
      );
      if (mounted) {
        setState(() {
          _currentPosition = position;
      ...
      if (kDebugMode) {
        print('LostDogReportPage - Current position: ${_currentPosition?.latitude ?? 41.0103}, ${_currentPosition?.longitude ?? 28.6724}');
      }
    } catch (e) {
      if (mounted) {
        setState(() {
      ...
      if (kDebugMode) print('LostDogReportPage - Error getting location: $e, using default position');
      if (e is TimeoutException) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Location timed out. Please try again or enter manually.')),
        );
      }
    }
  }

  Future<void> _sendNotification(String title, String body, String lostDogId) async {
    ...
    try {
      ...
      if (kDebugMode) print('LostDogReportPage - Notification sent successfully via Cloud Function');
    } catch (e) {
      if (kDebugMode) print('LostDogReportPage - Error sending notification via Cloud Function: $e');
    }
  }

  Future<void> _submitReport() async {
    if (kDebugMode) print('LostDogReportPage - Submitting report, _currentPosition: $_currentPosition, _isSubmitting: $_isSubmitting');
    if (_formKey.currentState!.validate() && !_isSubmitting) {
      if (_currentPosition == null) {
        if (kDebugMode) print('LostDogReportPage - Position is null, fetching new location...');
        await _getCurrentLocation();
        if (_currentPosition == null) {
          if (kDebugMode) print('LostDogReportPage - Still no position, using default');
        }
      }
      ...
      try {
        ...
        if (kDebugMode) {
          print('LostDogReportPage - Submitting LostDog: $lostDog');
          print('LostDogReportPage - toMap: ${lostDog.toMap()}');
        }

        final docRef = await FirebaseFirestore.instance.collection('lost_dogs').add(lostDog.toMap());
        if (kDebugMode) print('LostDogReportPage - Report submitted successfully with doc ID: ${docRef.id}');

        await _sendNotification(
          'New Lost Dog Reported',
          '${lostDog.name} (${lostDog.breed}) has been reported lost near ${lostDog.latitude}, ${lostDog.longitude}',
          docRef.id,
        );
        ...
        if (kDebugMode) print('LostDogReportPage - Error submitting report: $e');
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (kDebugMode) print('LostDogReportPage - Form validation failed or submitting');
    }
  }
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      _buildTextField(_lostLocationController, localizations.lostLocationLabel, required: true),
                      const SizedBox(height: 16),
