...
class _FoundDogReportPageState extends State<FoundDogReportPage> {
  ...
  Future<void> _getCurrentLocation() async {
    try {
      ...
      if (kDebugMode)
        debugPrint(
          'FoundDogReportPage - Current position: ${_currentPosition!.latitude}, ${_currentPosition!.longitude}',
        );
    } catch (e) {
      if (mounted) {
        setState(() {
      ...
      if (kDebugMode)
        debugPrint(
          'FoundDogReportPage - Error getting location: $e, using default position',
        );
    }
  }
  ...
  Future<void> _submitReport() async {
    ...
    if (_formKey.currentState!.validate() && !_isSubmitting) {
      ...
      try {
        ...
        if (kDebugMode)
          debugPrint(
            'FoundDogReportPage - Report submitted successfully with doc ID: ${docRef.id}',
          );

        await _sendNotification(
          title: 'Found Pet Reported',
          body:
      ...
      if (kDebugMode)
        debugPrint('FoundDogReportPage - Form validation failed or submitting');
    }
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextFormField(
                          ...
                          keyboardType:
                              _selectedContactType == "Phone" ||
                                  _selectedContactType == "WhatsApp"
                              ? TextInputType.phone
...
class FoundMapPickerPage extends StatefulWidget {
  ...
  const FoundMapPickerPage({super.key, required this.initialLocation});

  @override
  State<FoundMapPickerPage> createState() => _FoundMapPickerPageState();
}

class _FoundMapPickerPageState extends State<FoundMapPickerPage> {
